...
<template>
  <DialogRoot v-model:open="open">
    <DialogPortal>
      ...
      >
        <div class="grid shrink-0 gap-1.5 pr-8">
          <DialogTitle class="text-lg font-semibold leading-none tracking-tight">{{
            title
          }}</DialogTitle>
          <DialogDescription
            :class="description ? 'text-sm leading-relaxed text-muted-foreground' : 'sr-only'"
            >{{ description || title }}</DialogDescription
          >
        </div>
        <div class="min-h-0 flex-1 overflow-y-auto p-1.5"><slot /></div>
        <footer
          v-if="$slots.footer"
          class="flex shrink-0 flex-col-reverse gap-2 border-t pt-4 sm:flex-row sm:justify-end"
        >
          <slot name="footer" />
        </footer>
        <DialogClose
