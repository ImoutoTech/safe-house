...
- Do not pass that OIDC interaction continuation as `return_to` when starting a normal external login. The provider callback URI is server-owned and must remain stable; resume the interaction from tab-scoped `sessionStorage` after the callback result is exchanged. Identity-binding start requests keep their own server-supported `return_to` contract.
